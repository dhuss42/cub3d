...
void draw_rays(t_cub *cub, int x)
{
    ...
    float step = 0.01; // Small increment for smooth ray drawing
    ...
    while (1)
    {
        ...
        mlx_put_pixel(cub->image, pixel_x, pixel_y, 0xFF0000FF);
...
void    draw_walls(t_cub *cub, int x)
{
    ...
    {
        // printf("test7\n");
        mlx_put_pixel(cub->image, x +(WIDTH / 2), y, cub->colour);
        y++;
    }
}
